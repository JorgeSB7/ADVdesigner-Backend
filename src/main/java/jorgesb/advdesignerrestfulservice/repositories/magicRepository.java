...
public interface magicRepository extends JpaRepository<magic, Long> {
    ...
    @Query(value = "SELECT * FROM magics AS i WHERE i.creatorm=?1", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM magics AS i WHERE i.namemagic LIKE %?1% AND i.creatorm=?2", nativeQuery = true)
